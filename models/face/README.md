...
PicPortal uses one versioned embedding space on macOS and on the server:
...
production macOS bundle can be built without downloading executable model data
...
artifact is tracked and agrees with this manifest before CI compiles it.
...
and SHA-256 validation on macOS or Linux. It is intentionally an out-of-image
restoration tool: the face-service image copies the reviewed, committed ONNX
weights, licenses, and parity fixture, verifies them without network access,
and loads both ONNX sessions during the image build.

Canonical preprocessing is documented in `docs/macos-local-media-pipeline.md`.
...
Rust pins `ort` 2.0.0-rc.10 and its statically linked ONNX Runtime 1.22.0.
Python pins ONNX Runtime 1.24.4. This intentional runtime skew is accepted only while both
implementations exceed the manifest's `0.999` cosine-parity floor on the same
fixture; CI executes both paths on macOS.
